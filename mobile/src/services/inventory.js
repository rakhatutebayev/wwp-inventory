...
export const inventoryService = {
  ...
  // Отметка записи по ID
  checkRecord: async (recordId, notes = null) => {
    const data = notes ? { notes } : {}
    const response = await api.post(`/api/inventory/records/${recordId}/check`, data)
    return response.data
  },
  
  // Снять отметку проверки
  uncheckRecord: async (recordId) => {
    const response = await api.post(`/api/inventory/records/${recordId}/uncheck`)
    return response.data
  },
}
